...
struct cache_element{// linked list type of datatype
...
cache_element* find(char* url); // to search the element in cache
int add_cache_element(char* data,int size,char* url); // add the request in cache 
void remove_cache_element(); // remove the request from cache
